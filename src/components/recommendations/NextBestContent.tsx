...
export function NextBestContent({
  sessionId,
  contentItemId,
  canonicalUrl,
  title = "Следующий шаг",
  className,
}: NextBestContentProps) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/recommendations/next-content?${params.toString()}`)
      .then((r) => r.json())
  ...
  if (loading) return <p className="text-sm text-muted">Загрузка следующего шага…</p>;
  if (!items.length) return <RecommendationEmptyState title="Нет следующего шага" className={className} />;
