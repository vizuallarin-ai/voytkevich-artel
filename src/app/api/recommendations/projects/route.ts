...
export async function GET(request: Request) {
  ...
  const rate = checkRateLimit(`recommendations-projects:${ip}`, { limit: 60, windowMs: 60_000 });
  if (!rate.ok) {
  ...
  const items = await recommendationOrchestratorService.getProjectRecommendations(context);
